...
export class TaskService {
  ...
  public updateIsCompleteStatus(taskId:string, isCompleted:boolean) : Observable<Task> {
    return this._httpClient.patch<Task>(`${this._apiUrl}/tasks/${taskId}`, {isCompleted}
    );
  }
  ...
  public updateATaskInTheTasksList(updatedTask: Task): void {
    this.tasks.update(tasks => {
      const allTasksWithUpdatesTaskRemoved = tasks.filter(
        task => task.id != updatedTask.id
      );

      const updatedTaskList = [...allTasksWithUpdatesTaskRemoved, updatedTask];
